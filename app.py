...
if page == "Prediksi Individu":
    ...
    if submitted:
        ...
        if pred == "Inactive":
            ...
            # ➡️ Dua saran berbeda tergantung status membership
            if membership == "Yes":   # Pelanggan BARU tetapi SUDAH member
                st.info(tr(
                    "📌 Saran: Undang mereka mengikuti *Barista Experience Tour* gratis, sertakan kupon **upgrade ukuran minuman** sekali pakai, dan kirim info event komunitas tertutup.",  # Indonesia
                    "📌 Tip: Invite them to a complimentary *Barista Experience Tour*, include a **free size-up coupon**, and share details of members-only community events."                      # English
                ))
            else:                     # Pelanggan BARU dan BELUM menjadi member
                st.info(tr(
                    "📌 Saran: Tawarkan paket *Starter Membership*—welcome drink gratis, **double stempel digital** pertama, dan voucher diskon 20 % untuk kunjungan berikutnya.",  # Indonesia
                    "📌 Tip: Offer a *Starter Membership* bundle—free welcome drink, **double digital stamps** on the first visit, and a 20 % voucher for the next purchase."       # English
                ))
